import { splitObject1, splitObject2, splitObject3, splitObject4, } from './alina/sandbox.js';


for (const solution of [splitObject1, splitObject2, splitObject3, splitObject4,]) {
describe('Splits an object into multiple objects with one key/value pair each.', () => {
  ...
  describe('object values are special values', () => {
    ...
    it('should return a array with one key/value pair', () => {
      ...
      const expected = [ { name: 'null' }, { age: Infinity }, { weight: undefined } ];
      const actual = solution(toSeparate);
      expect(actual).toEqual(expected);
    });
    it('should return a array with one key/value pair', () => {
